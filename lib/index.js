...
var ResizeableTitle = function (Iprops) {
    var onResize = Iprops.onResize, width = Iprops.width, onClick = Iprops.onClick, restProps = __rest(Iprops, ["onResize", "width", "onClick"]);
    if (!width) {
...
// 拖拽调整table
var ResizeTable = function (props) {
    var ref = props.ref, _components = props.components;
    ...
    var resizeCell = function (text, key) {
        return (React.createElement("span", { key: key, className: 'ellipsis-text', style: { width: '100%' } }, text));
    };
    ...
    var genCols = function (columns) {
        var _a, _b;
        var tableInstanceClientWidth;
        if (ref) {
            tableInstanceClientWidth = (_a = ref.current) === null || _a === void 0 ? void 0 : _a.clientWidth;
        }
        else {
            tableInstanceClientWidth = (_b = tableRef.current) === null || _b === void 0 ? void 0 : _b.clientWidth;
        }
    ...
    var refreshResizeTable = function () {
        var _cols = (cols || []).map(function (col, index) {
            var width = col.width;
            return __assign(__assign({}, col), { width: width, onHeaderCell: function (column) { return ({
                    width: column.width,
                ...
                render: col.render
                    ? function () {
                        ...
                        return React.createElement("div", { className: "ellipsis-text" }, col.render.apply(col, args));
                    }
                    : function (text) { return resizeCell(text, col.key); } });
        });
        setColumns(_cols);
    };
    useEffect(function () {
    ...
    return (React.createElement("div", { className: 'resize-table' },
        React.createElement(Table, __assign({ ref: ref || tableRef, size: "small", scroll: { x: 0 }, tableKey: 'key' }, props, { components: components, columns: columns }))));
};
